 ...
 */
open class FloatWindowLauncher(val context: Context) {
    private val tag = FloatWindowLauncher::class.java.simpleName

    private val serviceIntent: Intent by lazy {
        val intent = Intent(MultiFloatWindowConstants.ACTION_SERVICE_CORE)
        intent.`package` = MultiFloatWindowConstants.PACKAGE_NAME_CORE
        intent
    }
    ...
    private val mConnection = object: ServiceConnection {
        ...
        override fun onServiceConnected(name: ComponentName?, binder: IBinder?) {
            Log.i(tag, "onServiceConnected name=$name, binder=$binder")
            if(binder != null) {
                mServiceMessenger = Messenger(binder)
                // 接続できた時に処理するコールバックが必要！！、onServiceConnectedはすぐに発火しない。
                this@FloatWindowLauncher.onServiceConnected(name, binder)
            }
        }
    }
